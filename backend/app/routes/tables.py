...
from app.models.table import TableCreate, TableUpdate, TableResponse, TableStatusUpdate
...
async def create_table(table: TableCreate, username: str = Depends(verify_token)):
    ...
    result = await table_collection.insert_one(table_dict)
    new_table = await table_collection.find_one({"_id": result.inserted_id})
...
async def update_table(
    ...
    update_data = {k: v for k, v in table.dict(exclude_unset=True).items() if v is not None}
